...
<div class="relative min-h-screen group-data-[sidebar-size=sm]:min-h-sm">
    <div class="group-data-[sidebar-size=lg]:ltr:md:ml-vertical-menu group-data-[sidebar-size=lg]:rtl:md:mr-vertical-menu group-data-[sidebar-size=md]:ltr:ml-vertical-menu-md group-data-[sidebar-size=md]:rtl:mr-vertical-menu-md group-data-[sidebar-size=sm]:ltr:ml-vertical-menu-sm group-data-[sidebar-size=sm]:rtl:mr-vertical-menu-sm pt-[calc(theme('spacing.header')_*_1)] pb-[calc(theme('spacing.header')_*_0.8)] px-4">
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center">
                <div class="grow">
                    <h5 class="text-16">Unit Management</h5>
                </div>
            ...
            <div class="card" id="unitTable">
                <div class="card-body">
                    <div class="flex items-center gap-3 mb-4">
                        <h6 class="text-15 grow">Units (<b class="total-units">{{ $units->count() }}</b>)</h6>
                        <div class="shrink-0">
                            <a href="#!" data-modal-target="addUnitModal" type="button" class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600">
                                <i data-lucide="plus" class="inline-block size-4"></i> <span class="align-middle">Add Unit</span>
                            </a>
                        </div>
                    </div>
                                ...
                                <tr>
                                    ...
                                    <td class="px-3.5 py-2.5">
                                        <div class="flex gap-3">
                                            <a href="#!" data-id="{{ $unit->id }}" data-name="{{ $unit->name }}" data-description="{{ $unit->description }}" data-status="{{ $unit->status }}" data-modal-target="editUnitModal" class="edit-unit-btn flex items-center justify-center rounded-md bg-slate-100 text-slate-500 hover:text-custom-500 hover:bg-custom-100">
                                                <i data-lucide="pencil" class="size-4"></i>
                                            </a>
                                            <a href="#!" data-id="{{ $unit->id }}" data-modal-target="deleteModal" class="delete-unit-btn flex items-center justify-center rounded-md bg-slate-100 text-slate-500 hover:text-custom-500 hover:bg-custom-100">
    ...
    <!-- Add Unit Modal -->
    <div id="addUnitModal" modal-center="" class="fixed flex flex-col hidden transition-all duration-300 left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show">
        <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
            <div class="flex items-center justify-between p-4 border-b dark:border-zink-500">
                <h5 class="text-16">Add Unit</h5>
                <button data-modal-close="addUnitModal" class="transition-all duration-200 ease-linear text-slate-400 hover:text-red-500">
                    <i data-lucide="x" class="size-5"></i>
                </button>
            </div>
            <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
                <form action="{{ route('unit.store') }}" method="POST" id="create-unit-form">
                    @csrf
                    <div class="grid grid-cols-1 gap-4 xl:grid-cols-12">
                        <div class="xl:col-span-12">
                            <label for="unitName" class="inline-block mb-2 text-base font-medium">Name</label>
                            <input type="text" id="unitName" name="name" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500" placeholder="Enter unit name" required>
                        </div>
                        <div class="xl:col-span-12">
                            <label for="unitDescription" class="inline-block mb-2 text-base font-medium">Description</label>
                            <textarea id="unitDescription" name="description" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500" placeholder="Enter description"></textarea>
                        </div>
                        <div class="xl:col-span-12">
                            <label for="unitStatus" class="inline-block mb-2 text-base font-medium">Status</label>
                            <select id="unitStatus" name="status" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500">
                    ...
                    <div class="flex justify-end gap-2 mt-4">
                        <button type="reset" data-modal-close="addUnitModal" class="text-red-500 bg-white btn hover:text-red-500 hover:bg-red-100">Cancel</button>
                        <button type="submit" class="text-white btn bg-custom-500 border-custom-500 hover:bg-custom-600">Add Unit</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <!-- Edit Unit Modal -->
    <div id="editUnitModal" modal-center="" class="fixed hidden transition-all duration-300 left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4">
        <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
            <div class="flex items-center justify-between p-4 border-b dark:border-zink-500">
                <h5 class="text-16" id="editUnitLabel">Edit Unit</h5>
                <button data-modal-close="editUnitModal" id="closeEditUnitModal" class="transition-all duration-200 ease-linear text-slate-400 hover:text-red-500">
                    <i data-lucide="x" class="size-5"></i>
                </button>
            </div>
            <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
                <form id="edit-unit-form" method="POST" action="">
                    ...
                    <div class="grid grid-cols-1 gap-4 xl:grid-cols-12">
                        <div class="xl:col-span-12">
                            <label for="editUnitName" class="inline-block mb-2 text-base font-medium">Name</label>
                            <input type="text" id="editUnitName" name="name" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200" placeholder="Unit name">
                        </div>
                        <div class="xl:col-span-12">
                            <label for="editUnitDescription" class="inline-block mb-2 text-base font-medium">Description</label>
                            <textarea id="editUnitDescription" name="description" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200" placeholder="Unit description"></textarea>
                        </div>
                        <div class="xl:col-span-12">
                            <label for="editUnitStatus" class="inline-block mb-2 text-base font-medium">Status</label>
                            <select id="editUnitStatus" name="status" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                                <option value="Available">Available</option>
                                <option value="Occupied">Occupied</option>
                                <option value="Maintenance">Maintenance</option>
                            </select>
                    ...
                    <div class="flex justify-end gap-2 mt-4">
                        <button type="reset" data-modal-close="editUnitModal" class="text-red-500 bg-white btn hover:text-red-500 hover:bg-red-100">Cancel</button>
                        <button type="submit" id="saveEditUnit" class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600">Save Changes</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <!-- Delete Unit Modal -->
    <div id="deleteUnitModal" modal-center="" class="fixed hidden transition-all duration-300 left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4">
        <div class="w-screen md:w-[25rem] bg-white shadow rounded-md dark:bg-zink-600">
            <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto px-6 py-8">
                <div class="float-right">
                    <button data-modal-close="deleteUnitModal" id="closeDeleteUnitModal" class="transition-all duration-200 ease-linear text-slate-500 hover:text-red-500">
                        <i data-lucide="x" class="size-5"></i>
                    </button>
                </div>
                ...
                <div class="mt-5 text-center">
                    ...
                    <p class="text-slate-500 dark:text-zink-200">Are you certain you want to delete this unit?</p>
                    <form id="delete-unit-form" method="POST" action="">
                        @csrf
                        @method('DELETE')
                        <input type="hidden" id="deleteUnitId" name="unit_id">
                        <div class="flex justify-center gap-2 mt-6">
                            <button type="reset" data-modal-close="deleteUnitModal" class="bg-white text-slate-500 btn hover:text-slate-500 hover:bg-slate-100 focus:text-slate-500 focus:bg-slate-100 active:text-slate-500 active:bg-slate-100 dark:bg-zink-600 dark:hover:bg-slate-500/10 dark:focus:bg-slate-500/10 dark:active:bg-slate-500/10">
                                Cancel
                            </button>
                            <button type="submit" id="confirmDeleteUnit" class="text-white bg-red-500 border-red-500 btn hover:text-white hover:bg-red-600 hover:border-red-600 focus:text-white focus:bg-red-600 focus:border-red-600 focus:ring focus:ring-red-100 active:text-white active:bg-red-600 active:border-red-600 active:ring active:ring-red-100 dark:ring-custom-400/20">
                                Yes, Delete It!
                            </button>
                        </div>
                    </form>
...
<script>
    document.querySelectorAll('.edit-unit-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            const unitDescription = this.dataset.description;
            const unitStatus = this.dataset.status;
            ...
            // Update form action
            document.getElementById('edit-unit-form').action = `/unit/update/${unitId}`;
    ...
    document.querySelectorAll('.delete-unit-btn').forEach(button => {
        button.addEventListener('click', function() {
            // Get the unit ID from the button's data attributes
            const unitId = this.dataset.id;

            // Set the unit ID in the hidden input field
            document.getElementById('deleteUnitId').value = unitId;

            // Update the form action dynamically
            const form = document.getElementById('delete-unit-form');
            form.action = `/unit/delete/${unitId}`;
